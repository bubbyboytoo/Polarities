...
namespace Polarities.Content.NPCs.Bosses.Hardmode.ConvectiveWanderer
{
    ...
    public class ConvectiveWanderer : ModNPC
    {
                ...
                case -3:
                    {
                        ...
                        player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = 2;
                        player.GetModPlayer<PolaritiesPlayer>().screenshakeTimer = 60; // 1 seconds
                ...
                case 0:
                    {
                        ...
                        if (NPC.ai[1] < part1Time)
                        {
                            ...
                            player.GetModPlayer<PolaritiesPlayer>().screenshakeMagnitude = 1;
                            player.GetModPlayer<PolaritiesPlayer>().screenshakeTimer = 30;
                        }
                        else
                        {
                            Idle(radius: 600f);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            if (DrawLayer.IsActive<DrawLayerAdditiveAfterLiquids>())
            {
                ...
                {
                    Rectangle frame = StarConstruct.DashTexture.Value.Frame();
                    Vector2 origin = new Vector2(StarConstruct.DashTexture.Value.Width, StarConstruct.DashTexture.Value.Height / 2);

                    float scale = 8 * (1 - instadashTelegraphProgress);

                    spriteBatch.Draw(StarConstruct.DashTexture.Value, NPC.Center + new Vector2(280, 0).RotatedBy(NPC.rotation) - screenPos, frame, ModUtils.ConvectiveFlameColor((float)Math.Pow(instadashTelegraphProgress * 0.5f + 0.5f, 2)) * (0.5f / scale), NPC.rotation + MathHelper.Pi, origin, new Vector2(16, scale), SpriteEffects.None, 0f);
                }
            }
            else
            {
    ...
    {
        public override string Texture => "Polarities/Content/Items/Weapons/Ranged/Flawless/ContagunProjectile";
    ...
    {
        public override string Texture => "Polarities/Assets/Glow256";
